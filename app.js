...
let cart = JSON.parse(localStorage.getItem('aliBurgerCart')) || [];
...
function updateCart() { const count = cart.reduce((sum, item) => sum + item.qty, 0); const total = cart.reduce((sum, item) => sum + item.price * item.qty, 0); document.querySelector('#cart-count').textContent = count; document.querySelector('#cart-sum').textContent = `${total} KGS`; document.querySelector('#cart-total').textContent = `${total} KGS`; document.querySelector('#cart-items').innerHTML = cart.length ? cart.map(item => `<article class="cart-row"><div><h3>${item.name}</h3><p>${item.price} KGS за шт.</p><div class="quantity"><button onclick="changeQty('${item.name}', -1)">−</button><b>${item.qty}</b><button onclick="changeQty('${item.name}', 1)">+</button></div></div><div><b>${item.price * item.qty} KGS</b><button class="remove" aria-label="Удалить" onclick="removeItem('${item.name}')">×</button></div></article>`).join('') : '<p class="empty">Себет азырынча бош.<br>Менюдан даамдуу нерсе тандаңыз!</p>'; localStorage.setItem('aliBurgerCart', JSON.stringify(cart)); }
...
function orderWhatsApp() { const message = cart.length ? `Заказ ALI BURGER:%0A${cart.map(item => `${item.name} × ${item.qty} — ${item.price * item.qty} KGS`).join('%0A')}%0A%0AЖалпы сумма: ${cart.reduce((sum, item) => sum + item.price * item.qty, 0)} KGS%0A%0AПожалуйста, укажите ваш номер телефона для связи.` : 'Здравствуйте! Хочу сделать заказ в ALI BURGER. Пожалуйста, укажите ваш номер телефона для связи.'; window.open(`https://wa.me/996999170013?text=${encodeURIComponent(message)}`, '_blank'); }
function openMap() { document.querySelector('#map-modal').classList.add('open'); document.body.style.overflow = 'hidden'; }
function closeMap() { document.querySelector('#map-modal').classList.remove('open'); document.body.style.overflow = ''; }
render();
updateCart();
