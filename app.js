...
function orderWhatsApp() { const message = cart.length ? `Заказ ALI BURGER:%0A${cart.map(item => `${item.name} × ${item.qty} — ${item.price * item.qty} KGS`).join('%0A')}` : 'Здравствуйте! Хочу сделать заказ в ALI BURGER.'; window.open(`https://wa.me/996999170013?text=${encodeURIComponent(message)}`, '_blank'); }
render();
updateCart();
